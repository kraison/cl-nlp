...
;;; (William Bland's asdf-installable version):
;;;
;;; 1.02 -- 5th Jan 2007
;;;         Added unstem function, for getting back to inputs from stems.
...
(defparameter *keep-unstems-p*
  nil
  "If true, keep a record of the shortest 'unstem' for each stem that has been computed.")

(defparameter *unstems*
  (make-hash-table :test #'equal)
  "A mapping from stems to input words.  Only maintained if *keep-unstems-p* is true.")

(defun unstem (stem)
  "Retrieve the shortest input that has resulted in the given stem, or nil if none has."
  (gethash stem *unstems*))
...
(defun stem (str)
  (let ((input (copy-seq str))
    ...
    (when (and *keep-unstems-p*
	       (or (null (gethash str *unstems*))
		   (< len (length (gethash str *unstems*)))))
      (setf (gethash str *unstems*) input))
    str))
