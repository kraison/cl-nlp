...
(defmethod read-wordnet-synsets ((language language) &key (path "prolog")
  ...
  (with-open-file (in (format nil "~A/~A" path file))
	...
	(let* ((length (length line))
	       (end (- length 2)))
	  (when (> length 0)
	    (assert (string= line "s(" :end1 2))
	    (assert (string= line ")." :start1 end))
	    (destructuring-bind (id word-number quoted-word ss-type
				    &optional sense-number tag-count)
		(split-sequence:split-sequence #\, line :start 2 :end end)
...
(defmethod read-glosses ((language language) &key (path "prolog") (file "wn_g.pl"))
  (with-open-file (in (format nil "~A/~A" path file))
	...
	(let* ((length (length line))
	       (end (- length 2)))
	  (when (> length 0)
	    (assert (string= line "g(" :end1 2))
	    (assert (string= line ")." :start1 end))
	    (multiple-value-bind (ss-id offset)
		(split-sequence:split-sequence #\, line :start 2 :end end :count 1)
	      (incf count)
	      (let ((gloss (subseq line (1+ offset) (1- end)))
		    (synset-id (parse-integer (car ss-id))))
...
(defmethod read-syntactic-markers ((language language) &key (path "prolog")
  ...
  (with-open-file (in (format nil "~A/~A" path file))
	...
	(let* ((length (length line))
	       (end (- length 2)))
	  (when (> length 0)
	    (assert (string= line "syntax(" :end1 7))
	    (assert (string= line ")." :start1 end))
	    (destructuring-bind (ss-id word-number marker)
		(split-sequence:split-sequence #\, line :start 7 :end end)
	      (incf count)
...
(defmethod read-wordnet-pointerfile ((language language) file prolog-op slot-name
  ...
  (with-open-file (in file)
    (let ((count 0)
	  ...
	  (let* ((length (length line))
		 (end (- length 2)))
	    (assert (string= line prolog-op :end1 op-length))
	    (assert (string= line "(" :start1 op-length :end1 (1+ op-length)))
	    (assert (string= line ")." :start1 end))
	    (destructuring-bind (ss-id-1 ss-id-2)
		(split-sequence:split-sequence #\, line :start (1+ op-length) :end end)
	      (incf count)
